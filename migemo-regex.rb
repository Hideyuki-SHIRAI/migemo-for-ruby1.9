...
module MigemoRegex
  ...
  class RegexEmacsshyMetachars < RegexMetachars
    def initialize
      @bar = '\\|'
      @lparen = '\\(?:'
      @rparen = '\\)'
  ...
  class RegexRenderer
    ...
    def insert (string)
      string = string.force_encoding('euc-jp')
      if @insertion != ""
	...
	render_alternation(x).force_encoding('euc-jp')
      elsif x.instance_of?(RegexConcatnation)
	render_concatnation(x).force_encoding('euc-jp')
      elsif x.instance_of?(RegexCharClass)
	render_charclass(x).force_encoding('euc-jp')
      elsif x.instance_of?(String)
	render_string(x).force_encoding('euc-jp')
      else
  ...
  class RegexEmacsshyRenderer < RegexRenderer
    def initialize (regex, insertion)
      super(regex, insertion)
      @meta = RegexEmacsshyMetachars.new
    end
  ...
  module RegexRendererFactory
	...
	RegexEmacsRenderer.new(regex, insertion)
      when "emacsshy"
	RegexEmacsshyRenderer.new(regex, insertion)
      when "perl"
